...
Enemy* Enemy::nodeWithTheGame(GameMain* game)
{
...
bool Enemy::initWithTheGame(GameMain* game)
{
	bool bRet = false;
	do
	{
...
void Enemy::getAttacked(Hero* attacker)
{
	attackedBy->addObject(attacker);
}

void Enemy::gotLostSight(Hero* attacker)
{
	attackedBy->addObject(attacker);
}
